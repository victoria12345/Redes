...
y los vuelca a traza nueva (�correctamente?) con tiempo actual
 ...
 Compila: gcc -Wall -o ejemplo ejemplo.c -lpcap
...
void fa_nuevo_paquete(uint8_t *usuario, const struct pcap_pkthdr* cabecera, const uint8_t* paquete){
	...
	int i;
	struct pcap_pkthdr* aux;
	
	aux = (struct pcap_pkthdr*)malloc(sizeof(struct pcap_pkthdr));
	aux->ts.tv_sec = cabecera->ts.tv_sec + 1800;
	aux->ts.tv_usec = cabecera->ts.tv_sec;
...
/*	mirar esto*/
printf("Tamanno: %d\n", sizeof(paquete));
	for(i = 0; i < n_bytes && i < sizeof(paquete); i++){
		printf("%x ", paquete[i]);
	}
	printf("\n");
	if(pdumper){
