...
int main(int argc, char **argv)
{
	...
	if(argc < 2){
		printf("Argumentos insuficientes.\n
				Ejecute con un argumento si desea capturar de interfaz.\n
				Ejecute con dos argumentos si quiere analizar una traza pcap.");
		exit(ERROR);
	}
	...
	if(argc = 2){
		//Si se ejecuta con 1 argumento abrimos la interfaz para la captura
		if ((descr = pcap_open_live("eth0",5,0,100, errbuf)) == NULL){
		printf("Error: pcap_open_live(): %s, %s %d.\n",errbuf,__FILE__,__LINE__);
		exit(ERROR);
		}
	}else if(argc = 3){
		//Si se ejecuta con 2 argumentos abrimos el fichero pcap
